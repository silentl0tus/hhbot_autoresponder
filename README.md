...
> [!WARNING]
> **Системное требование для РФ: Обязательное наличие VPN**
> 1. Доступ к API передовых нейросетей (Google Gemini, Claude, OpenRouter) ограничен из РФ. Поэтому **на вашем ПК должен быть запущен системный VPN** (например, Hiddify, NekoRay, v2ray, Amnezia). 
>    - Бот автоматически подхватывает ваш системный VPN без дополнительных настроек. Любые конфликты с устаревшими протоколами (SOCKS4) бот обходит сам — он просто будет работать через ваш включенный VPN.
> 2. 🔴 **Критичный нюанс (Раздельное туннелирование):** Трафик к сайтам **hh.ru** и **api.hh.ru** должен идти **с российского IP-адреса**. HeadHunter блокирует зарубежные IP (Cloudflare 403). Убедитесь, что ваш VPN настроен так, чтобы пускать локальные (RU) сайты напрямую (например, режим «Проксировать только зарубежный трафик» или маршрутизация по GeoIP в Hiddify).
...
- 🤖 **100% Бесплатный AI-слой:**
  ...
  - **Умный Fallback:** Если основная модель падает от перегрузки (ошибки 503 или 429), бот автоматически переключается на массив запасных бесплатных моделей, гарантируя непрерывность работы.
  - Расширенные задачи: генерация ответов на тесты и анкеты HR-скринеров (разворачивание вашей короткой идеи в вежливый ответ), анализ тональности сообщений от работодателей и мультимодальное (Vision) решение капчи.
...
- 🎯 **Мультиплатформенность и высокоточный парсинг:**
  - **HeadHunter (hh.ru):** Детальная информация запрашивается через официальный API `api.hh.ru/vacancies/<id>` по OAuth-токену, что исключает блокировки Cloudflare и дает ИИ полный контекст.
  - **Хабр Карьера:** Полноценная поддержка парсинга и откликов на ИТ-платформе Хабра.
  - **MAX (Скринеры):** Бот поддерживает интеграцию и авторизацию для прохождения скринеров в мессенджере MAX.
...
- **Python:** 3.12+ (В Windows бот устанавливает его сам).
- **Telegram:** Созданный бот в [@BotFather](https://t.me/BotFather) и ваш числовой ID из [@userinfobot](https://t.me/userinfobot).
- **Сеть:** Включенный VPN (для пользователей из РФ).
- **Аккаунты:** hh.ru, Хабр Карьера (опционально) с опубликованным резюме.
- **AI-ключ (бесплатный):**
...
#### На Windows (Максимально просто):
1. Запустите двойным кликом **`run_windows.bat`**. 
   - Скрипт **полностью автоматизирован**: он сам проверит наличие Python на вашем ПК. Если Python (версии 3.12+) нет, скрипт автоматически скачает официальный установщик и установит его в тихом режиме!
   - Затем скрипт сам создаст виртуальное окружение и установит все зависимости (включая драйверы браузеров).
2. Запустите **`login_windows.bat`** — в открывшемся окне браузера войдите в hh.ru. Сессия сохранится автоматически.
   - (Для Хабр Карьеры используйте `login_habr_windows.bat`, для скринеров MAX — `login_max_windows.bat`).
...
1. Создайте пустую Google Таблицу. Для быстрого старта импортируйте в неё готовый шаблон **`google_sheets_template.csv`** (находится в корне папки `hh-autoresponder`), либо вручную создайте колонки: `Дата`, `Ссылка`, `Позиция`, `Компания`, `Контакт`, `CV`, `CL`, `Статус`, `Комментарий`.
...
- **Ошибка `Unknown scheme for proxy URL URL('socks4://...')`:**
  Вы используете системный VPN (например, Hiddify, v2ray), который прописывает `socks4` в систему. Начиная с версии 1.3, бот **автоматически игнорирует** эту ошибку и работает напрямую через ваш VPN. Если ошибка всё же появляется, убедитесь, что вы обновили файлы репозитория (`git pull`).
