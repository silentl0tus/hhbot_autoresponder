...
graph TD
    ...
    subgraph "Разработка и Автотесты"
        P[Pytest] -.->|Покрытие тестами логики парсеров и AI| Q((CI/Локально))
    end
```
...
* **`parsers/`** — Ядро взаимодействия с hh.ru и Хабр Карьерой:
  - `hh_api.py`, `habr.py` (быстрый поиск и отклики через API)
  - `hh_playwright.py`, `habr_playwright.py` (тяжелая работа: обработка сложных анкет, сопроводительных писем и тестов через headless-браузер)
...
* **`tests/`** — Набор автотестов (Pytest) для проверки работы фильтров, обработчиков бота и парсеров.
