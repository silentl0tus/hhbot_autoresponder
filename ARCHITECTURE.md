...
* **Скрипты запуска (`*.bat` / `*.sh`)**: 
  - Настроены на максимальную отказоустойчивость: в Windows (`run_windows.bat`) автоматически проверяется наличие Python, скачивается официальный установщик, настраивается UTF-8 и обходятся баги кодировки командной строки (chcp 65001 bug).
